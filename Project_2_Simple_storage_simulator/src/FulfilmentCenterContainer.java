...
public class FulfilmentCenterContainer {
    ...
    void summary(){
        ...
        while(it.hasNext()){
            Map.Entry<String, FulfilmentCenter> entryFulfilment = it.next();
            System.out.println(String.format("Name: %s Fill %.2f proc.", entryFulfilment.getKey(), new Double(((entryFulfilment.getValue().howManyProductIHave()/entryFulfilment.getValue().maxCapacitance)*100))));
        }
    }
}
